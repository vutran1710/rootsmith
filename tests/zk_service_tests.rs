...
async fn test_rootsmith_flow_partner_to_zk_service() {
    ...
    if !plugin_path.exists() {
        ...
        println!("   Run: cd tests/plugins && cargo build --target wasm32-unknown-unknown --release");
        return;
    }
    ...
    let wasm_host = WasmPluginHost::load(plugin_path_str, limits)
    ...
    let rootsmith = RootSmith::new(
        ...
        ArchiveStorageVariant::Noop(rootsmith::archiver::NoopArchive),
        config,
        storage,
    )
    ...
    let wasm_host_guard = rootsmith.wasm_host.as_ref()
        .expect("WASM host not configured");
    let mut wasm_host = wasm_host_guard.lock().await;
    let output: Box<dyn ToStandardData> = wasm_host.process_input(partner_json.as_bytes())
    ...
    match &request.data {
        InputData::RawBytes(rows) => {
            println!("  Data: RawBytes ({} rows, {} bytes per row)", rows.len(), rows.first().map(|r| r.len()).unwrap_or(0));
            println!("  Total data size: {} bytes", rows.iter().map(|r| r.len()).sum::<usize>());
        }
